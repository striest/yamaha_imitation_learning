...
if __name__ == '__main__':
    gmm = make_gmm(n=40, std=0.01, r=1.0)
...
#        fake_samples = (torch.rand(batch_size, 2) - 0.5) * 4.
        ...
        ## compute WGAN grad penalty ##
        alpha = torch.rand(batch_size, 1)
        interp = alpha * real_samples2 + (1.-alpha) * fake_samples2
        interp = torch.autograd.Variable(interp, requires_grad=True)
        d_interp = discriminator.forward(interp)
        ...
        ## generator update is min[-D(G(z))]##
        if i % 5 == 0:
            gen_objective = -fake_sample_logits
            loss = gen_objective.mean()
            generator_opt.zero_grad()
            loss.backward()
            generator_opt.step()
        ## discriminator update is min[-D(x) + D(G(z)) + reg]##
        else:
            ## compute gradient penalty ##
            disc_objective = -real_sample_logits + fake_sample_logits
            loss = disc_objective.mean() + g_lam * grad_penalty.mean()
        ...
        print('Avg. Disc. real - fake: {:.4f}'.format(real_sample_logits.mean() - fake_sample_logits.mean()))
