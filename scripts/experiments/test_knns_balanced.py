...
from yamaha_imitation_learning.dataset.balanced_dataset import BalancedDataset
...
def make_learners(train_dataset, img_feature_extractor, device):
    return {
        'k5_s1_expweight': VisualKNN(train_dataset, img_feature_extractor, k=5, dataset_skip=1, aggregate='expweight').to(device),
        'k10_s1_expweight': VisualKNN(train_dataset, img_feature_extractor, k=10, dataset_skip=1, aggregate='expweight').to(device),
        'k20_s1_expweight': VisualKNN(train_dataset, img_feature_extractor, k=20, dataset_skip=1, aggregate='expweight').to(device),
        'k50_s1_expweight': VisualKNN(train_dataset, img_feature_extractor, k=50, dataset_skip=1, aggregate='expweight').to(device),
        ...
        'k50_sanity_check_random': SanityCheck(train_dataset, k=20, dataset_skip=1, aggregate='random').to(device),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--distill_p', type=float, required=False, default=0.1, help='percentage of dataset to use for eval')
    ...
    balanced_train_dataset = BalancedDataset(train_dataset, img_feature_extractor, distill_p=args.distill_p)
    balanced_test_dataset = BalancedDataset(test_dataset, img_feature_extractor, distill_p=args.distill_p)

    cost_net = torch.load(args.cost_net_fp).to(args.device)
    cost_net.network.eval()

    learners = make_learners(balanced_train_dataset, img_feature_extractor, args.device)
    for learner in learners.values():
        learner.process_dataset()

    #set up logging
    os.mkdir(args.save_fp)
    os.mkdir(os.path.join(args.save_fp, 'all_data_eval'))
    os.mkdir(os.path.join(args.save_fp, 'balanced_data_eval'))
    for lk in learners.keys():
        for ds_type in ['all_data_eval', 'balanced_data_eval']:
            os.mkdir(os.path.join(args.save_fp, ds_type, lk))
            os.mkdir(os.path.join(args.save_fp, ds_type, lk, 'learner'))
            os.mkdir(os.path.join(args.save_fp, ds_type, lk, 'results'))
    ...
    for i in range(len(test_dataset)):
        ...
        for lk, lv in learners.items():
            ...
            plt.savefig(os.path.join(args.save_fp, 'all_data_eval', lk, 'learner', '{:05d}_learner.png'.format(i)))
            plt.close()

            visualize(dpt, intermediate)
            plt.savefig(os.path.join(args.save_fp, 'all_data_eval', lk, 'results', '{:05d}_results.png'.format(i)))
            plt.close()

    final_res = {k:{kk:torch.tensor(vv) for kk,vv in v.items()} for k,v in final_res.items()}
    torch.save(final_res, os.path.join(args.save_fp, 'all_data_eval', 'metrics.pt'))

    final_res = {vk:{k:[] for k in metrics.keys()} for vk in learners.keys()}
    for i in range(len(balanced_test_dataset)):
        print('{}/{}'.format(i, len(balanced_test_dataset)))
        dpt = balanced_test_dataset[i]
        for lk, lv in learners.items():
            with torch.no_grad():
                res, intermediate = evaluate_metrics(dpt, lv, metrics, model, cost_net, metadata)
            ...
            lv.visualize(query=dpt)
            plt.savefig(os.path.join(args.save_fp, 'balanced_data_eval', lk, 'learner', '{:05d}_learner.png'.format(i)))
            plt.close()

            visualize(dpt, intermediate)
            plt.savefig(os.path.join(args.save_fp, 'balanced_data_eval', lk, 'results', '{:05d}_results.png'.format(i)))
            plt.close()

    final_res = {k:{kk:torch.tensor(vv) for kk,vv in v.items()} for k,v in final_res.items()}
    torch.save(final_res, os.path.join(args.save_fp, 'balanced_data_eval', 'metrics.pt'))
